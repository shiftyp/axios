 ...
 */
export default function dispatchRequest<T = any>(config: InternalAxiosRequestConfig): Promise<AxiosResponse<T>> {
  ...
  if (['post', 'put', 'patch'].indexOf(config.method.toLowerCase()) !== -1) {
    config.headers.setContentType('application/x-www-form-urlencoded', false);
  }

  const adapter = adapters.getAdapter(config.adapter || defaults.adapter);

  return adapter(config).then(function onAdapterResolution(response: AxiosResponse<T>) {
    throwIfCancellationRequested(config);

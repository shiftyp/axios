...
describe('core::mergeConfig', function() {
  ...
  ['auth', 'headers', 'params', 'proxy'].forEach(function(key) {
    it('should set new config for' + key + ' without default', function() {
      const a: Record<string, any> = {}, b: Record<string, any> = {}, c: Record<string, any> = {};
      a[key] = undefined;
  ...
  describe('valueFromConfig2Keys', function() {
    const config1: AxiosRequestConfig = {url: '/foo', method: 'post', data: {a: 3}};
  ...
  describe('defaultToConfig2Keys', function() {
    ...
    it('should prefer config2 if config1 is undefined', function() {
      expect(mergeConfig({transformRequest: undefined}, {transformRequest: [1, 2, 3]})).toEqual({transformRequest: [1, 2, 3]});
    });

    it('should prefer config1 if config2 is undefined', function() {
      expect(mergeConfig({transformRequest: [1, 2, 3]}, {transformRequest: undefined})).toEqual({transformRequest: [1, 2, 3]});
    });

    it('should use config2 if both are defined', function() {
      expect(mergeConfig({transformRequest: [1, 2, 3]}, {transformRequest: [4, 5, 6]})).toEqual({transformRequest: [4, 5, 6]});
    });
  });
